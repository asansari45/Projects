...
bool File::Write(Value v)
{
    ...
    Header hdr;
    hdr.m_Type = SymbolTable::SymbolInfo::ATOMIC;
...
bool File::Write(ArrayValue v)
{
    // Write the header first
    Header hdr;
    hdr.m_Type = SymbolTable::SymbolInfo::ARRAY;
    if (v.GetType() == typeid(int))
    {
        hdr.m_SubType = INTEGER;
    }
...
bool File::Read(SymbolTable::SymbolInfo::SymbolType& rType, Value& rValue, ArrayValue& rArrayValue)
{
    ...
    if (hdr.m_Type == SymbolTable::SymbolInfo::ARRAY)
    {
        std::vector<int> dims;
        int elementCount = 1;
        for (int i=0; i < 3; i++)
