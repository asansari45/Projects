...
namespace Interpreter
{
    ...
    void NoSymbolLvalue::Equ(Rvalue& rRvalue)
    {
        ...
        {
            info.m_Type = SymbolTable::SymbolInfo::ARRAY;
            info.m_ArrayValue = rRvalue.GetArrayValue();
            status = m_pSymbolTable->CreateSymbol(m_Name, info);
        }
        else if (rRvalue.GetType() == SymbolTable::SymbolInfo::ATOMIC)
        {
            info.m_Type = SymbolTable::SymbolInfo::ATOMIC;
            info.m_Value = rRvalue.GetValue();
            status = m_pSymbolTable->CreateSymbol(m_Name, info);
        }
        else if (rRvalue.GetType() == SymbolTable::SymbolInfo::FILE)
        {
            info.m_Type = SymbolTable::SymbolInfo::ATOMIC;
            info.m_Filename = rRvalue.GetFile();
            status = m_pSymbolTable->CreateSymbol(m_Name, info);
        }
        assert(status);
    }
    ...
    void NoSymbolLvalue::Dim(Rvalue& rRvalue)
    {
        assert(m_pSymbolTable != nullptr);
        assert(m_pSymbolTable->IsSymbolPresent(m_Name) == false);
        assert(rRvalue.GetType() == SymbolTable::SymbolInfo::ARRAY);
        SymbolTable::SymbolInfo info;
        info.m_Name = m_Name;
        info.m_Type = SymbolTable::SymbolInfo::ARRAY;
        info.m_ArrayValue = rRvalue.GetArrayValue();
